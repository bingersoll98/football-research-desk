(function () {
  ...
  function mergeBook(book) {
    ...
    if (!Array.isArray(b.bets)) b.bets = [];
    if (Array.isArray(b.flyer) && b.flyer.length) {
      b.flyer.forEach((r) => {
        r.flyer = true;
        if (!b.bets.some((x) => x.id === r.id || (x.game === r.game && x.play === r.play))) b.bets.push(r);
      });
    }
    b.flyer = [];
    return b;
  ...
  async function writeBook(book) {
    if (!session) return;
    const clean = mergeBook(book);
    if (!cloud) {
      const db = loadLocal();
      if (db.users[session.email]) { db.users[session.email].book = clean; session.book = clean; db.session = session; saveLocal(db); }
      return;
    }
    await sb.from("books").upsert({ user_id: session.id, data: clean, updated_at: new Date().toISOString() });
  }
  ...
  async function login() {
    ...
    if (cloud) {
      ...
      session = { email, name: u.name, book: mergeBook(u.book) }; db.users[email].book = session.book; db.session = session; saveLocal(db);
    }
    showApp();
  }
  ...
  async function boot() {
    ...
    if (cloud && window.supabase) {
      ...
      const db = loadLocal();
      if (db.session && db.session.email && db.users[db.session.email]) { session = db.session; session.book = mergeBook(db.users[db.session.email].book); showApp(); return; }
    }
    showGate();
  }
  document.addEventListener("click", async (e) => {
    ...
    if (t.id === "save-row") {
      const book = await currentBook(); const row = readForm("bets", book); if (!row) return;
      if (window._edit) { const i = book.bets.findIndex((x) => x.id === window._edit); if (i >= 0) { row.id = window._edit; book.bets[i] = row; } window._edit = null; }
      else book.bets.unshift(row);
      await writeBook(book); render();
    }
    const ed = t.getAttribute("data-edit");
    if (ed) { const book = await currentBook(); window._edit = ed; $("form-wrap").classList.remove("hidden"); $("form-wrap").innerHTML = rowForm("bets", book.bets.find((x) => x.id === ed)); }
    const del = t.getAttribute("data-del");
    if (del && confirm("Remove this row?")) { const book = await currentBook(); book.bets = book.bets.filter((x) => x.id !== del); await writeBook(book); render(); }
  });
  boot();
})();
